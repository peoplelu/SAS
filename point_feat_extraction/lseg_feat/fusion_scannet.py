...
def get_args():
    ...
    parser.add_argument('--output_dir', type=str, default='scannet_multiview_lseg', help='output data path')
...
def main(args):   
    ...
    # to export full resolution training set point cloud
    ...
    data_paths = sorted(glob(join(data_root, split, '*.pth')))  # datapath only contain train/val/test according to split
    ...
    for i in trange(total_num):
        ...
        print('current scene: ' + data_paths[i])
