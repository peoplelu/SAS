...
class GeneralizedSEEM(nn.Module):
    ...
    def evaluate(self, batched_inputs):
        ...
        targets = targets_grounding = queries_grounding = None
        features = self.backbone(images.tensor)
        outputs, mask_emb = self.sem_seg_head(features, target_queries=queries_grounding)
        ...
        for mask_cls_result, mask_pred_result, box_pred_result, input_per_image, image_size in zip(
            ...
            # print('semantic_on: ', self.semantic_on)
            if self.semantic_on:
            ...
            # print('panoptic_on: ', self.panoptic_on)
            if self.panoptic_on:
                panoptic_r = retry_if_cuda_oom(self.panoptic_inference)(mask_cls_result, mask_pred_result)
                processed_results[-1]["panoptic_seg"] = panoptic_r
            
            # # instance segmentation inference
            # print('instance_on: ', self.instance_on)
            # if self.instance_on:
        ...
        return processed_results, mask_emb
    ...
    def panoptic_inference(self, mask_cls, mask_pred):
        ...
        if cur_masks.shape[0] == 0:
            ...
            return panoptic_seg, segments_info, real_id_list, keep
        else:
            # take argmax
