...
from fusion_util import extract_seem_img_feature, PointCloudToImageMapper, save_fused_feature, adjust_intrinsic, make_intrinsic
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description='Multi-view feature fusion of SEEM on ScanNet.')
    parser.add_argument('--data_dir', type=str, default='scannet', help='scannet data path')
    parser.add_argument('--output_dir', type=str, default='scannet_multiview_seem', help='save data path')
    ...
    # parser.add_argument('--process_id_range', nargs='+', default=['801, 1500'], help='the id range to process')
...
def process_one_scene(data_path, out_dir, args):
    ...
    for img_id, img_dir in enumerate(tqdm(img_dirs)):
        ...
        if keep_features_in_memory:
            ...
            # this way
            feat_2d = extract_seem_img_feature(img_dir, transform, evaluator, clip_pretrained)
            # torch.cuda.empty_cache()
            if feat_2d == None:
                continue
            feat_2d = feat_2d.to(device)
