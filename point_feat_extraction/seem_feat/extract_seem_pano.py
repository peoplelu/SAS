...
for scene in os.listdir(path):
    ...
    for img in os.listdir(scene_path):
        ...
        mask_path = img_path[:23] + '/pano_seg/' + img_name + '.pth'
        mask_img_path = img_path[:23] + '/pano_seg_img/' + img_name + '.png'
        ...
        if not os.path.exists(img_path[:23] + '/pano_seg'):
            os.mkdir(img_path[:23] + '/pano_seg')  

        if not os.path.exists(img_path[:23] + '/pano_seg_img'):
            os.mkdir(img_path[:23] + '/pano_seg_img')  
        ...
        colors = np.random.randint(0, 256, (len(pano_seg_info)+1, 3))

        sem_seg_img = colors[pano_seg.cpu().numpy()]

        plt.imshow(sem_seg_img.astype(np.uint8))


        torch.save(pano_seg, mask_path)
