...
from run.distill_sp import get_model
...
def get_parser():
    ...
    parser = argparse.ArgumentParser(description='SAS evaluation')
...
def evaluate(model, val_data_loader, labelset_name='scannet_3d'):
    ...
    with torch.no_grad():
        ...
        for rep_i in range(args.test_repeats):
            ...
            for i, (coords, feat, label, feat_3d, mask, inds_reverse, inds, superpoint, scene_name) in enumerate(tqdm(val_data_loader)):
