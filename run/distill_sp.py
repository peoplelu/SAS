...
from dataset.feature_loader import collation_fn_eval_all as collation_fn
...
def get_parser():
    ...
    parser = argparse.ArgumentParser(description='SAS 3D distillation: First Stage.')
...
def distill(train_loader, model, optimizer, epoch):
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        (coords, feat, label_3d, feat_3d, mask, inds_recons, vox_ind, superpoint, scene_name) = batch_data
        ...
        feat_3d, mask = feat_3d.cuda(
            non_blocking=True).float(), mask.cuda(non_blocking=True)
        ...
        if output_3d.shape[0] <= 20000:
            superpoint = superpoint[vox_ind][mask]
            _, superpoint = torch.unique(superpoint, sorted=True, return_inverse=True)

            feat_3d_mean = scatter_mean(feat_3d, superpoint, dim=0)
            feat_3d_mean_f = feat_3d_mean[superpoint, :]

            loss_1 = (1 - torch.nn.CosineSimilarity()(output_3d, feat_3d_mean_f)).mean()
            
            output_3d = scatter_mean(output_3d, superpoint, dim=0)

            loss_2 = (1 - torch.nn.CosineSimilarity()(output_3d, feat_3d_mean)).mean()
            ...
            feat_3d = feat_3d[rand_ind]

            superpoint = superpoint[vox_ind][mask][rand_ind]
            _, superpoint = torch.unique(superpoint, sorted=True, return_inverse=True)
