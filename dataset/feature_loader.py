...
class FusedFeatureLoader(Point3DLoader):
    ...
    def __init__(self,
                 ...
                 input_color=False, return_sp=False, return_name=False, thres=0.1
                 ):
    ...
    def __getitem__(self, index_long):
        ...
        if self.return_sp:
            superpoint = torch.load('data/scannet_3d/superpoint/'+scene_name+'_vh_clean_2.pth')
            superpoint = torch.from_numpy(superpoint)
            assert superpoint.shape[0] == locs_in.shape[0]
        ...
        if 'nuscenes' not in self.dataset_name:
            ...
            # processed_data = torch.load(join(
            #     self.datapath_feat, scene_name+'_0.pt'))
        else:
            # no repeated file
        ...
        if len(processed_data.keys())==2:
            ...
            feat_3d, mask_chunk = feat_3d.detach(), mask_chunk.detach()
        ...
        locs = locs_in
        locs_in = self.prevoxel_transforms(locs_in) if self.aug else locs_in
        ...
        if self.return_sp and self.return_name:
            return coords, feats, labels, feat_3d, mask, torch.from_numpy(inds_reconstruct).long(), vox_ind, \
            superpoint, scene_name
        elif self.return_sp:
            return coords, feats, labels, feat_3d, mask, torch.from_numpy(inds_reconstruct).long(), vox_ind, superpoint
        elif self.return_name:
            return coords, feats, labels, feat_3d, mask, torch.from_numpy(inds_reconstruct).long(), vox_ind, scene_name
        else:
            return coords, feats, labels, feat_3d, mask, torch.from_numpy(inds_reconstruct).long(), vox_ind
...
def collation_fn(batch, return_sp=False, return_name=False):
    '''
    :param batch:
    ...
    if return_sp and return_name:
        coords, feats, labels, feat_3d, mask_chunk, superpoint, scene_name = list(zip(*batch))
        for i in range(len(coords)):
            coords[i][:, 0] *= i
        return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
            torch.cat(feat_3d), torch.cat(mask_chunk), torch.cat(superpoint), scene_name
    elif return_sp:
        coords, feats, labels, feat_3d, mask_chunk, superpoint = list(zip(*batch))
        for i in range(len(coords)):
            coords[i][:, 0] *= i
        return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
            torch.cat(feat_3d), torch.cat(mask_chunk), torch.cat(superpoint)
    elif return_name:
        coords, feats, labels, feat_3d, mask_chunk, scene_name = list(zip(*batch))
        for i in range(len(coords)):
            coords[i][:, 0] *= i
        return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
            torch.cat(feat_3d), torch.cat(mask_chunk), scene_name
    else:
...
def collation_fn_eval_all(batch, return_sp=True, return_name=True):
    '''
    :param batch:
    ...
    if return_sp and return_name:
        coords, feats, labels, feat_3d, mask, inds_recons, vox_ind, superpoint, scene_name = list(zip(*batch))
    elif return_sp:
        coords, feats, labels, feat_3d, mask, inds_recons, vox_ind, superpoint = list(zip(*batch))
    elif return_name:
        coords, feats, labels, feat_3d, mask, inds_recons, vox_ind, scene_name = list(zip(*batch))
    else:
        coords, feats, labels, feat_3d, mask, inds_recons, vox_ind = list(zip(*batch))


    inds_recons = list(inds_recons)
    vox_ind = list(vox_ind)
    superpoint = list(superpoint)

    accmulate_inds_recons = 0
    accmulate_vox_ind = 0
    accmulate_superpoint = 0

    for i in range(len(coords)):
        coords[i][:, 0] *= i

        inds_recons[i] = accmulate_inds_recons + inds_recons[i]
        vox_ind[i] = accmulate_vox_ind + vox_ind[i]
        superpoint[i] = accmulate_superpoint + superpoint[i]

        assert coords[i].shape[0] == vox_ind[i].shape[0]
        accmulate_inds_recons += coords[i].shape[0]
        assert inds_recons[i].shape[0] == superpoint[i].shape[0]
        accmulate_vox_ind += superpoint[i].shape[0]
        accmulate_superpoint = superpoint[i].max() + 1

    
    if return_sp and return_name:
        return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
            torch.cat(feat_3d), torch.cat(mask), torch.cat(inds_recons), \
            torch.cat(vox_ind), torch.cat(superpoint), scene_name
    elif return_sp:
        return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
            torch.cat(feat_3d), torch.cat(mask), torch.cat(inds_recons), \
            torch.cat(vox_ind), torch.cat(superpoint)
    elif return_name:
        return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
            torch.cat(feat_3d), torch.cat(mask), torch.cat(inds_recons), \
            torch.cat(vox_ind), scene_name
    else:
        return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
            torch.cat(feat_3d), torch.cat(mask), torch.cat(inds_recons), \
            torch.cat(vox_ind)
...
def collation_fn_eval_all_offset(batch, return_sp=True, return_name=True):
    '''
    :param batch:
    :return:    coords: N x 4 (x,y,z,batch)
                feats:  N x 3
    ...
    offset = [i.shape[0] for i in inds_recons]
    ...
    if return_sp and return_name:
        return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
            torch.cat(feat_3d), torch.cat(mask), torch.cat(inds_recons), \
            torch.cat(vox_ind), torch.cat(superpoint), scene_name, offset
    elif return_sp:
        return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
            torch.cat(feat_3d), torch.cat(mask), torch.cat(inds_recons), \
            torch.cat(vox_ind), torch.cat(superpoint)
