...
class RandomDropout(object):
    ...
    def __call__(self, coord, feat, label, feat_3d, mask, superpoint):
        if random.random() < self.dropout_application_ratio:
            n = coord.shape[0]
            idx = np.random.choice(n, int(n * (1 - self.dropout_ratio)), replace=False)

            feat_3d_new = torch.zeros((coord.shape[0], feat_3d.shape[1]), dtype=feat_3d.dtype)
            feat_3d_new[mask] = feat_3d

            mask = mask[idx]

            feat_3d = feat_3d_new[idx][mask]

            coord = coord[idx]
            feat = feat[idx]
            label = label[idx]

            _, superpoint = torch.unique(superpoint[idx], sorted=True, return_inverse=True)

        return coord, feat, label, feat_3d, mask, superpoint
...
class RandomRotate(object):
    ...
    def __call__(self, coord):
        ...
        if self.center is None:
            ...
            center = self.center
        coord -= center
        coord = np.dot(coord, np.transpose(rot_t))
        coord += center
...
class RandomJitter(object):
    ...
    def __call__(self, coord):
        jitter = np.clip(
            self.sigma * np.random.randn(coord.shape[0], 3),
            -self.clip,
            self.clip,
        )
        coord += jitter
