...
def collation_fn_eval_all(batch):
    ...
    '''
    coords, feats, labels, inds_recons, superpoint, scene_name = list(zip(*batch))
    ...
    return torch.cat(coords), torch.cat(feats), torch.cat(labels), torch.cat(inds_recons), torch.cat(superpoint), scene_name
...
class Point3DLoader(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index_long):
        ...
        if self.use_shm:
            ...
            if 'scannet_3d' in self.data_paths[index]:
                scene_name = self.data_paths[index].split('/')[-1][0:12]
            else:
                scene_name = self.data_paths[index].split('/')[-1][0:12]
        ...
        if self.eval_all:
            return coords, feats, labels, torch.from_numpy(inds_reconstruct).long(), superpoint, scene_name
